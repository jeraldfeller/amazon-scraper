...
if(isset($_GET['action'])){
    ...
    $data = $scraper->getAsin($table, $limit);
//	var_dump($data); 
   $rowCount = 0;
    $context = stream_context_create(array(
        'http' => array(
            'header' => array('User-Agent: Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36'),
        ),
    ));
    foreach($data as $row){
        $id = $row['id'];
